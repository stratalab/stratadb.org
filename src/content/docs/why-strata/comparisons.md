...
title: "Comparisons"
section: "why-strata"
description: "How Strata sits next to SQLite, DuckDB, Redis, Postgres, and dedicated vector databases."
source: "strata-core@v1.0.0"
---

Strata borrows its shape — embedded, single-file-ish, zero-config — from SQLite
...
Every other system on that list does one shape well. Strata's bet is different:
...
- Need **billion-scale, heavily-tuned vector search as the whole product**? A
  ...
  events, and a graph, branched together? That is Strata's lane.
- Need **branch-isolated, versioned, multi-model state**? That is what Strata is
  for — see [When to use Strata](/docs/why-strata/when-to-use).

For the honest boundaries of what Strata is *not*, see
[When to use Strata](/docs/why-strata/when-to-use); for the FAQ versions of these
