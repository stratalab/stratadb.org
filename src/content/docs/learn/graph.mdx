...
names the component each node belongs to, which is the quickest way to find out
whether your graph is one thing or several. Two component names here, so two
islands:
...
The count itself is in `--json` as `component_count`, not in the table.
...
**Deleting a populated graph is refused until you say so.**
`strata graph delete social` on a graph that still holds nodes or edges fails
with `failed_precondition.engine.graph_not_empty` and deletes nothing; pass
`--force` to drop its contents along with it. This matches `space delete`,
which has always refused while a space holds data. Graph delete asked for
nothing before 1.2.5.
