...
function referenceSection(entries: Doc[]): NavSection {
  ...
  for (const fam of families) {
    const commands = entries
      .filter((e) => e.id.startsWith(`reference/${fam}/`))
      .sort((a, b) => a.data.title.localeCompare(b.data.title));
    const index = entries.find((e) => e.id === `reference/${fam}`);
    groups.push({
      title: familyTitle(fam),
      href: index ? `/docs/reference/${fam}` : undefined,
      items: commands.map((e) => ({ title: e.data.title, href: `/docs/${e.id}` })),
    });
  }
  return {
    ...
    items: topLevel
      .sort((a, b) => {
        const pa = REFERENCE_PAGE_ORDER.indexOf(a.id);
        const pb = REFERENCE_PAGE_ORDER.indexOf(b.id);
        if (pa !== -1 || pb !== -1) return (pa === -1 ? 99 : pa) - (pb === -1 ? 99 : pb);
        return a.data.title.localeCompare(b.data.title);
      })
      .map((e) => ({ title: e.data.title, href: hrefFor(e.id, 'reference') })),
...
export async function docsNav(): Promise<NavSection[]> {
  ...
  for (const dir of SECTION_ORDER) {
    const entries = docs.filter((d) => d.id.startsWith(`${dir}/`) || d.id === dir);
    ...
    entries.sort((a, b) => {
      const pa = preferred.indexOf(a.id);
      const pb = preferred.indexOf(b.id);
      if (pa !== -1 || pb !== -1) return (pa === -1 ? 99 : pa) - (pb === -1 ? 99 : pb);
      return (
    ...
    sections.push({
      ...
      items: entries.map((e) => ({ title: e.data.title, href: hrefFor(e.id, dir) })),
    });
  }

  // top-level singles (faq, troubleshooting, …)
  const sectionRoots = new Set(SECTION_ORDER);
  const singles = docs.filter(
    (d) => !d.id.includes('/') && d.id !== 'index' && !sectionRoots.has(d.id),
  );
  if (singles.length)
    sections.push({
      title: 'More',
      items: singles
        .sort((a, b) => a.data.title.localeCompare(b.data.title))
        .map((e) => ({ title: e.data.title, href: `/docs/${e.id}` })),
    });
